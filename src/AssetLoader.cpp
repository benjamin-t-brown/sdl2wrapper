...
void loadAssetsFromFile<Sprite>(const std::string& path)
{
	...
	if (!ifs)
	{
		std::cout << "[SDL2Wrapper] ERROR Cannot load image list: " + path << std::endl;
		return;
	}
	...
	{
		std::cout << "[SDL2Wrapper] ERROR Failed to parse sprite list: " << e.what() << std::endl;
...
void loadAssetsFromFile<Animation>(const std::string& path)
{
	...
	if (!ifs)
	{
		std::cout << "[SDL2Wrapper] ERROR Cannot load Animation list: " + path << std::endl;
		return;
	}
	...
	try
	{
		...
		while (std::getline(ifs, line))
		{
			...
			else if (line.size())
			{
				...
				{
					std::cout << "[SDL2Wrapper] ERROR Failed to load anim sprite for: " << animName << std::endl;
